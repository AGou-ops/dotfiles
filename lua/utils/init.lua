...
local function avoid_hjkl(mode, mov_keys)
	for _, key in ipairs(mov_keys) do
		...
		vim.keymap.set(mode, key, function()
			if count >= 12 then
				id = vim.notify("WARN: 别他喵一直用hjkl了！使用w/W/b/B/e/E/f/F代替。", vim.log.levels.WARN, {
					-- icon = "🤠",
					replace = id,
					keep = function()
						return count >= 5
...
ToggleHardMode()
vim.api.nvim_set_keymap("n", "<leader>th", ":lua ToggleHardMode()<CR>", { noremap = true, silent = true })
